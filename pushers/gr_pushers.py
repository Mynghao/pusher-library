...
class grGCA:
    ...
    def update_u_i(self, metric, xold, x, u_i, D_phys, B_phys, dt):
        ...
        up2h = metric.transform(
            np.einsum("n,ni->ni", ups_new_2, b_i), xmid2d, Idx.D, Idx.T
        )
    ...
    def update_xi(self, metric, x, uph, D_phys, B_phys, dt):
        ...
        while err > 10 ** (-8):
            ...
            first = ups * np.einsum(
                "nij,nj->ni", metric.hij(xmid2d), b_i
            ) / gamma + np.einsum("nij,nj->ni", metric.hij(xmid2d), vE_i)
        ...
        u_iper = 0
        kappa = 1 / np.sqrt(
            1 - np.einsum("nij,ni,nj->n", metric.hij(xmid2d), vE_i, vE_i)
        )
